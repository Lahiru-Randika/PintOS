...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    while (true){
      ...
      while (true){
        uint8_t a =input_getc();
        printf("%c", a);
        //Compare with the ASCII value of carriage return ('\r').
        if (a== 13){
          printf("\n");
          break;
        }else{
          //insert characters to the array
          array[i]= (char)a;
        }
        i=i+1;
      }
      ...
      //compare
      if (strcmp(array,"whoami")==0){
          printf("Name : Lahiru\nIndex : 210527J\n");
      }else if(strcmp(array,"shutdown")==0){
          shutdown_power_off ();
      }else if(strcmp(array,"time")==0){
          //System timer that ticks, by default, 100 times per second.
          printf("%lld s\n", timer_ticks()/100);
      }else if(strcmp(array,"ram")==0){
          //This line is given.
          printf ("Pintos booting with %'"PRIu32" kB RAM...\n",
          init_ram_pages * PGSIZE / 1024);
      }else if(strcmp(array,"thread")==0){
          thread_print_stats();
      }else if(strcmp(array,"priority")==0){
          printf("%d\n",thread_get_priority());
      }else if(strcmp(array,"exit")==0){
          printf("Exiting interactive shell ...Bye!");
          break;
      //if user inputs unauthorized command
      }else{
